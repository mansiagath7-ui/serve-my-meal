...
import { Eye, Pencil, Trash2, Search, Plus, X, Upload, Shield } from "lucide-react";
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 p-4 rounded-3xl text-center">
              ...
              <span className={`text-[10px] font-black uppercase ${cat.cookId ? "text-orange-500" : "text-blue-500"}`}>
                {cat.cookId ? "Chef Managed" : "System Base"}
              </span>
            </div>
            <div className="bg-gray-50 p-4 rounded-3xl text-center">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">
...
export default function Categories() {
  ...
  const filtered = categories.filter((c) =>
    ...
    (!c.cookId || c.cookId === user?._id)
  );

  const deleteCategory = async (id) => {
    if (
      window.confirm(
        "Are you sure? This will remove the category from the global list.",
      )
    ) {
      try {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-6 py-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-xs text-gray-400 font-medium">
            Manage your custom food sections or use system bases
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {filtered.map((cat) => {
          const isOwner = cat.cookId === user?._id;
          return (
            <div
              key={cat._id}
            ...
            >
              <div className="relative h-40 overflow-hidden">
                ...
                <div className="absolute top-4 left-6">
                   {!cat.cookId && (
                     <div className="flex items-center gap-1.5 bg-blue-500/90 backdrop-blur-md text-white text-[9px] font-black uppercase px-3 py-1 rounded-full shadow-lg border border-blue-400/50">
                        <Shield size={10} />
                        System Global
                     </div>
                   )}
                   {cat.cookId && (
                     <div className="flex items-center gap-1.5 bg-orange-500/90 backdrop-blur-md text-white text-[9px] font-black uppercase px-3 py-1 rounded-full shadow-lg border border-orange-400/50">
                        Chef Created
                     </div>
                   )}
                </div>
              ...
              <div className="px-6 py-5 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <span className={`text-[9px] font-black uppercase ${isOwner ? "text-green-500" : "text-gray-400"}`}>
                    {isOwner ? "Manageable" : "Read-Only"}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-9 h-9 rounded-xl bg-gray-50 text-gray-400 hover:bg-orange-500 hover:text-white flex items-center justify-center transition-all shadow-sm"
                  >
                    <Eye size={14} />
                  </button>
                  {isOwner && (
